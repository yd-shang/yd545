...
def expected_shortfall(returns, weights=None, alpha=0.025):
    if weights is not None:
        ...
        # Use raw returns if no weights are provided
...
portfolio_volatility_sharpe = np.sqrt(np.dot(optimal_weights_sharpe.T, np.dot(cov_matrix, optimal_weights_sharpe)))
sharpe_ratio_sharpe = (portfolio_return_sharpe - risk_free_rate) / portfolio_volatility_sharpe
es_sharpe = expected_shortfall(data.values, optimal_weights_sharpe)
rrrp_sharpe = (portfolio_return_sharpe - risk_free_rate) / es_sharpe 
...
sharpe_ratio_risk_adjusted = (portfolio_return_risk_adjusted - risk_free_rate) / portfolio_volatility_risk_adjusted
...
portfolio_comparison = pd.DataFrame({
    "Metric": ["Expected Return", "Volatility", "Sharpe Ratio", "Expected Shortfall (2.5%)", "Risk-Adjusted Ratio"],
    "Sharpe Ratio Portfolio": [portfolio_return_sharpe, portfolio_volatility_sharpe, sharpe_ratio_sharpe, es_sharpe, rrrp_sharpe],
    "Risk-Adjusted Portfolio": [portfolio_return_risk_adjusted, portfolio_volatility_risk_adjusted, sharpe_ratio_risk_adjusted, es_risk_adjusted, risk_adjusted_ratio]
})
