...
def negative_log_likelihood(params):
    ...
    nll = 0.5 * n * np.log(2 * np.pi * sigma_sq) + 0.5 * np.sum((y - y_pred) ** 2) / sigma_sq
    return nll

# Initial guesses for beta (OLS estimate) and sigma^2
beta_ols = np.linalg.inv(X.T @ X) @ X.T @ y 
sigma_sq_ols = np.sum((y - X @ beta_ols) ** 2) / len(y)  
initial_params = np.append(beta_ols, sigma_sq_ols) 
...
beta_mle = result.x[:-1]  
sigma_sq_mle = result.x[-1] 

print(f"MLE Estimated Intercept (beta0): {beta_mle[0]}")
print(f"MLE Estimated Slope (beta1): {beta_mle[1]}")
print(f"MLE Estimated Variance (sigma^2): {sigma_sq_mle}")
print(f"MLE Estimated Standard Deviation (sigma): {np.sqrt(sigma_sq_mle)}")
